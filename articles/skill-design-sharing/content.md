...
### 4.1 第一个亮点：Workflow 是"长出来"的，不是"写出来"的
...
> 当然手搓总要懂格式、要心细。所以我后面做了第二个 Skill `workflow-creator`，让 Agent 帮你把这件事也工程化（第五章会讲）。但**手搓这条路始终是开放的**——它没有被工具链锁死。CLI 也好、人也好、另一个 Agent 也好，只要产出的文件结构对，Workflow 引擎都认。

这种"**扩展能力 ≠ 修改代码**"的设计，是这套系统最容易被忽视、但生命力最强的一点。

### 4.2 第二个亮点：Workflow 与 Skill 同构，可以无缝互转

如果你仔细看，Workflow 的目录结构和一个 Skill 的目录结构几乎是镜像的：

| Skill | Workflow |
|-------|----------|
| `SKILL.md`（描述 + 入口） | `WorkFlow.md`（描述 + 入口） |
| `references/` 下的步骤文档 | `references/` 下的步骤文档 |
| 通过 CLI 触发 | 通过 CLI 触发 |
| 在 discover 时被扫描 | 在 discover 时被扫描 |
...
- ⛓️ **多步连锁自动执行**：Agent 提交一次数据，CLI 可能跑完后续 3 步才回来（详见 4.8）

这些能力让 Workflow 在"多步骤、有状态、跨会话"的复杂业务场景里碾压裸 Skill。下面就一个个展开。

### 4.3 用一句话总结 Workflow 在做什么
...
### 4.5 设计细节一：步进式披露——Agent 永远只看到当前一步
...
### 4.8 设计细节四：模板变量——步骤间的数据流由 CLI 管理
